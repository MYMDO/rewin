<#
.SYNOPSIS
    Скрипт для повністю автоматичного "online" перевстановлення Windows 10 (Версія 3.0).
.DESCRIPTION
    Фінальна, надійна та ідентпотентна версія. Скрипт перевіряє стан системи
    на кожному кроці і пропускає вже виконані етапи. Це дозволяє запускати його
    повторно після ручного виправлення проблем.
.NOTES
    Автор: Ваш досвідчений системний адміністратор
    Версія: 3.0 - Ідентпотентна
...
if ($tempVolume -and $tempVolume.FileSystemLabel -eq $TempPartitionLabel) {
    ...
    try {
        ...
        # Стиснення та створення розділу
...
if (Test-Path $autounattendXmlPath) {
    ...
    # XML-шаблон... (залишається без змін)
    $xmlContent = @"
